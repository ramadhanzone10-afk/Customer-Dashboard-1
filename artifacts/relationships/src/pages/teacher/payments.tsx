...
export default function TeacherPayments() {
  ...
  const grouped = useMemo(() => {
    const map = new Map<
      string,
      { kelas: string; rows: { p: Payment; student: User | undefined }[] }
    >();
  ...
  function remindAllUnpaid(rows: { p: Payment; student: User | undefined }[]) {
    const unpaid = rows.filter((r) => r.p.status === "unpaid");
    if (unpaid.length === 0) {
  ...
  function waLink(p: Payment, student: User | undefined): string | null {
    const phone = formatPhoneForWa(student?.phone);
    if (!phone || !student) return null;
    const text = encodeURIComponent(
      `Halo ${student.name}, ini pengingat dari MathClub: pembayaran SPP bulan ${formatMonth(
        p.month,
      )} sebesar ${formatCurrency(p.amount)} belum kami terima. Mohon segera melakukan pembayaran. Terima kasih.`,
    );
  ...
  return (
    <div className="space-y-6">
      <div className="grid md:grid-cols-4 gap-4">
        <Card>
          <CardContent className="p-4">
            ...
            <Progress
              value={totalExpected ? (totalCollected / totalExpected) * 100 : 0}
      ...
      <Tabs value={activeMonth} onValueChange={setActiveMonth}>
        ...
        {months.map((m) => (
          <TabsContent key={m} value={m} className="mt-4 space-y-4">
            ...
            {visibleGroups.map((g) => {
              const paid = g.rows.filter((r) => r.p.status === "paid");
              const pending = g.rows.filter((r) => r.p.status === "pending");
              const unpaid = g.rows.filter((r) => r.p.status === "unpaid");
              const collected = paid.reduce((a, r) => a + r.p.amount, 0);
              const expected = g.rows.reduce((a, r) => a + r.p.amount, 0);
                        ...
                        return (
                          ...
                          >
                            <div
                              ...
                              style={{
                                background: student?.avatarColor ?? "#6366f1",
                              }}
                            >
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="text-xs text-muted-foreground">
                                ...
                                {p.uploadedAt &&
                                  ` · diunggah ${formatDate(p.uploadedAt)}`}
                                {p.verifiedAt &&
                                  ` · lunas ${formatDate(p.verifiedAt)}`}
                              </div>
                            </div>
                                ...
                                <>
                                  ...
                                  {wa ? (
                                    ...
                                    <Button
                                      ...
                                      variant="outline"
                                      disabled
                                      title="Siswa belum mengisi nomor HP"
                                    >
                                      <MessageCircle className="h-3 w-3 mr-1" />
                                      WA
                                    </Button>
      ...
      <Dialog
        open={!!previewPayment}
        onOpenChange={(o) => !o && setPreviewPayment(null)}
      >
